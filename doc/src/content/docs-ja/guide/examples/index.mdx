...
### 0 を明示的な例外にする CSS z-index

プロジェクトでデフォルトの stacking context に `0` を使う場合は、オブジェクト形式で CSS ルールを有効にし、その素の値だけを許可します。それ以外の整数とトークンのない計算式は違反のままです:
...
`allowed` は負の値を含む整数を受け付け、重複したエントリは重複排除されます。素の整数を一切例外にしない場合は `zIndex: true` を使います。
...
すべてのコメント形式については[無視構文](../ignore-syntax/index.mdx)を参照してください。
