...
- `` className={`...`} `` テンプレートリテラルと複数行の `{...}` 属性式(静的な文字列とテンプレートセグメント)
...
上記の従来のコンパイル済み形式は引き続き有効です。`css.zIndex: { allowed: [0] }` からコンパイルした設定は `zIndex: true` を維持し、明示的な素の整数の例外用にオプションの `zIndexAllowed: ReadonlySet<number>` を追加します。`zIndex: true` 自体には例外がありません。
...
`CompiledCssConfig` は互換性のため `zIndex` を boolean のまま保持し、オブジェクト形式を使った場合はコンパイル済みの `zIndexAllowed?: ReadonlySet<number>` を持つことがあります。
