...
Works with any framework that uses static Tailwind class attributes in source files — React, Astro, plain HTML, and Vue/Svelte templates for their static `class="..."` attributes. Vue's `:class`/`v-bind:class` bindings and Svelte's `class:name` directives are not correctly extracted (see [Known Limitations](../../reference/limitations/index.mdx)). Integrate with:
