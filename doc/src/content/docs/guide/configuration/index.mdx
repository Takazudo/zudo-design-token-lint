...
| `semanticPrefixes` | `string[]` | Namespace prefixes for your semantic-token vocabulary — a value under a listed namespace has the namespace stripped and its remaining tail re-tested against the same rule (default: `["hgap-", "vgap-", "hsp-", "vsp-"]`) |
...
**Counter-intuitive direction — read this before changing the list.** `semanticPrefixes` is a namespace *declaration*, not an allowlist. **Adding** a prefix can **add** violations, because it exposes the numeric tail hiding behind a semantic-looking name — it never removes one. `p-hgap-2` is flagged once `hgap-` is listed; `p-hgap-sm` still passes either way. See [Changed in v2.0.0](#fields-changed-in-v2-0-0) below.
...
2. It's a **namespace match** when the matched entry ends in `-`, or is immediately followed by `-` in the value. Its **tail** is everything after the stripped namespace (and its `-`, if the entry didn't already carry one).
3. When more than one entry matches as a namespace, the **longest** one wins — this keeps the result independent of array order.
4. The tail is then judged exactly like a plain value would be for this rule: empty or `"0"` passes (the same zero bypass every spacing/sizing rule already has), a tail matching the rule's own numeric pattern is **flagged**, and anything else — a token name like `sm` or `2xs` — passes.
5. The strip happens **once**, with no recursion: `p-hgap-vgap-2` strips only the outer `hgap-`, leaving tail `vgap-2`, which is not numeric, so it passes.
6. Matching is **case-sensitive** (`p-HGAP-2` does not match a `hgap-` entry).

| Class | `semanticPrefixes` | Result | Why |
|---|---|---|---|
| `p-hgap-sm` | `["hgap-"]` | pass | tail `sm` isn't numeric |
| `p-hgap-2` | `["hgap-"]` | **FLAG** | tail `2` matches the rule's numeric pattern — a numeric scale wearing a semantic name |
| `p-hgap-2.5` | `["hgap-"]` | **FLAG** | a decimal tail still matches |
| `p-hgap-0` | `["hgap-"]` | pass | zero tail |
| `p-hgap-` | `["hgap-"]` | pass | empty tail |
| `p-hgap-2xs` | `["hgap-"]` | pass | digit-*leading* token name (`2xs`), not purely numeric |
...
1. **Rename the token** so it isn't a numeric scale in disguise (`p-hgap-2` → `p-hgap-sm`) — this is the point of the rule.
...
In 1.x, `semanticPrefixes` had no observable effect on any built-in rule — a value like `hgap-sm` already failed the numeric-spacing check on its own, allowlist or not (tracked as [#108](https://github.com/Takazudo/zudo-design-token-lint/issues/108)). v2 makes the field real by re-testing the tail after the strip instead of treating any prefix match as an automatic pass.
...
- **Semantic spacing tokens**: `p-hgap-sm`, `gap-vgap-xs`, `m-hgap-md` (classes with a non-numeric `hgap-*`/`vgap-*`/`hsp-*`/`vsp-*` tail — see [`semanticPrefixes`](#fields-semanticprefixes); a numeric tail like `p-hgap-2` is flagged, not allowed)
