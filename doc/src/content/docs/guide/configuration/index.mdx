...
| `css` | `object` | Opt-in CSS/SCSS declaration scanning: `{ zIndex?: boolean \| { allowed: number[] }, colorLiterals?, patterns? }` (all default-OFF) |
...
| Field | Type | Description |
|---|---|---|
| `zIndex` | `boolean \| { allowed: number[] }` | Flag raw integer and token-less `calc()` `z-index` values (default `false`). The object form enables the rule and allows the listed raw integers. |
...
Forbids raw integer `z-index` values, enforcing the [semantic z-index tier system](https://zudo-css-wisdom.takazudomodular.com/). Mirrors the allow/forbid table from the z-index strategy. `zIndex: true` rejects every raw integer, including `0`; use the object form when a project has explicit exceptions:
...
`allowed` must be an array of integers. Negative integers are valid, and duplicate entries are harmless (they are deduplicated when compiled). Unknown fields and fractional/non-numeric entries are configuration errors. The object form implies that the rule is enabled; `false` and an omitted `zIndex` keep it disabled.
...
A `calc()` counts as allowed only when it contains a `var()` reference; `calc(100 + 1)` (raw arithmetic) is still flagged. A trailing `!important` does not exempt a raw integer, but whitespace and `!important` are normalized before checking the numeric allowlist.
