...
The linter flags: `p-4`, `bg-gray-100`, `text-gray-900`, `mb-2`, `text-gray-600`, `mb-4`, `gap-2`, `px-3`, `py-1`, `bg-blue-600`. (`text-xs` is left alone by this config — a `text-{n}` pattern only matches numeric values, so it can never match a non-numeric size keyword like `xs`; catching `text-xs` would require an exact-match entry such as `"text-xs"` in `prohibited`.)
...
### CSS z-index with an explicit zero exception

If a project reserves `0` for the default stacking context, use the object form to enable the CSS rule while allowing that one raw value. Other integers and token-less calculations remain violations:
...
`allowed` accepts integers, including negative values; duplicate entries are deduplicated. Use `zIndex: true` when no raw integer should be exempted.
...
See [ignore syntax](../ignore-syntax/index.mdx) for all comment forms.
