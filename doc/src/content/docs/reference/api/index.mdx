...
- `` className={`...`} `` template literals and multiline `{...}` attribute expressions (static strings and template segments)
...
The legacy compiled shape above remains valid. Configs compiled from `css.zIndex: { allowed: [0] }` keep `zIndex: true` and add an optional `zIndexAllowed: ReadonlySet<number>` for the explicit raw-integer exemptions; `zIndex: true` itself has no exceptions.
...
```ts
interface CssConfig {
  zIndex?: boolean | { allowed: number[] }; // default false; object form enables and allows listed integers
  colorLiterals?: boolean; // default false
  patterns?: string[]; // CSS/SCSS globs the CLI scans, in addition to the Tailwind `patterns`
}
```

`CompiledCssConfig` keeps `zIndex` as a boolean for compatibility and may carry the compiled `zIndexAllowed?: ReadonlySet<number>` when the object form was used.
