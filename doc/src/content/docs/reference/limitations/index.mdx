...
This means the linter will silently miss prohibited classes inside ternaries rather than producing false positives. There is no workaround; consider using static class names with conditional logic instead:

```tsx
// Linted correctly
<div className={isActive ? "p-hgap-sm" : "m-vgap-md"}>
```
