...
Run on staged files with [lint-staged](https://github.com/lint-staged/lint-staged):
...
for (const file of files) {
  ...
  for (const v of result.violations) {
    console.log(`${file}:${v.line}:${v.column}  ${v.className}  ${v.reason}`);
    totalViolations++;
  }
}
...
See [ignore syntax](/docs/ignore-syntax) for all comment forms.
